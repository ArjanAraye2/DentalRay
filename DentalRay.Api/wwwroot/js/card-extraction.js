...
(() => {
 ...
 function renderRadiology(modal,data){const teeth=data.problemTeeth||[],general=data.generalFindings||"";modal.querySelector(".card-extraction-body").innerHTML=`<p class="card-extraction-warning">این تحلیل جایگزین نظر و تشخیص دندانپزشک نیست.</p>${general?`<section><h4>یافته‌های کلی</h4><pre>${esc(general)}</pre></section>`:""}<section><h4>دندان‌های نیازمند بررسی</h4>${teeth.length?teeth.map(t=>`<div class="radiology-finding"><strong>دندان ${esc(t.toothNumber??"؟")}</strong>${(t.findings||[]).map(x=>`<p>یافته: ${esc(x)}</p>`).join("")}${(t.previousWork||[]).map(x=>`<p>درمان قبلی: ${esc(x)}</p>`).join("")}${(t.dentistReview||[]).map(x=>`<p>بررسی دندانپزشک: ${esc(x)}</p>`).join("")}<small>اطمینان: ${esc(t.confidence||"نامشخص")}</small></div>`).join(""):"<p>مورد مشخصی گزارش نشد.</p>"}</section>`;modal.querySelector(".card-extraction-copy").onclick=async()=>navigator.clipboard.writeText(modal.querySelector(".card-extraction-body").innerText);}
 async function analyze(image){close();const modal=document.createElement("div");modal.id="cardExtractionModal";modal.className="card-extraction-overlay";modal.innerHTML='<div class="card-extraction-dialog" role="dialog" aria-modal="true"><header><div><h3>تحلیل رادیولوژی</h3><span>تحلیل همان تصویر انتخاب‌شده</span></div><button type="button" class="card-extraction-close">×</button></header><div class="card-extraction-body"><p>در حال تحلیل تصویر رادیولوژی...</p></div><footer><button type="button" class="secondary-button card-extraction-copy" disabled>کپی تحلیل</button><button type="button" class="card-extraction-done">بستن</button></footer></div>';document.body.appendChild(modal);modal.querySelector(".card-extraction-close").onclick=close;modal.querySelector(".card-extraction-done").onclick=close;modal.addEventListener("click",e=>{if(e.target===modal)close();});try{const r=await fetch(`/api/ai/images/${image.imageID}/analyze-radiology`,{method:"POST"});let x={};try{x=await r.json();}catch{}if(!r.ok||x.success===false)throw new Error(x.message||"تحلیل رادیولوژی انجام نشد.");renderRadiology(modal,x.analysis||{});modal.querySelector(".card-extraction-copy").disabled=false;}catch(e){modal.querySelector(".card-extraction-body").innerHTML=`<p class="error">${esc(e.message||"تحلیل رادیولوژی انجام نشد.")}</p>`;}}
 window.DentalRayImageAI={open,analyze,isDentalType:dentalType};
