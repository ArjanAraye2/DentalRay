(() => {
    ...
    function ensureUi(){
        ...
        section.innerHTML=`
          <div class="section-header"><div><h2>مدیریت انواع تصویر</h2><p>تعریف انواع رادیولوژی مانند OPG و CBCT</p></div></div>
          <form id="imageTypeAdminForm">
            <div class="form-grid"><div class="form-field"><label for="imageTypeAdminName">نام نوع تصویر</label><input id="imageTypeAdminName" maxlength="150" required /></div></div>
        ...
        section.querySelector("form").onsubmit=e=>{e.preventDefault();save();}; cancelButton.onclick=reset;
    ...
    function reset(){editing=null;nameInput.value="";saveButton.textContent="ذخیره";cancelButton.hidden=true;}
    async function load(){
        ...
        list.innerHTML=""; (x.imageTypes||[]).forEach(item=>{
            ...
            const edit=document.createElement("button");edit.type="button";edit.textContent="ویرایش";edit.onclick=()=>{editing=item;nameInput.value=item.imageTypeName;saveButton.textContent="ذخیره ویرایش";cancelButton.hidden=false;nameInput.focus();};
    ...
    async function save(){
        ...
        const body=editing?{imageTypeID:editing.imageTypeID,imageTypeName:name,isActive:editing.isActive}:{imageTypeName:name,isActive:true};
