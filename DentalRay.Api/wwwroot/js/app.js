...
E.patientSummaryDentalChart=byId("patientSummaryDentalChart");
...
async function selectStudyTab(study,tab,tabs,panel){
 ...
 if(selectedStudyID!==study.studyID)return;
 if(studyResult.status==="fulfilled"&&studyResult.value.r.ok){const x=studyResult.value.x,teeth=x.toothNumbers||x.study?.toothNumbers||[];if(window.DentalRayDentalChart)window.DentalRayDentalChart.render(E.patientSummaryDentalChart,teeth);}
 else if(window.DentalRayDentalChart)window.DentalRayDentalChart.render(E.patientSummaryDentalChart,[]);
