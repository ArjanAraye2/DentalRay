...
async function loadImageTypes(){E.uploadImageType.innerHTML=`<option value="">انتخاب نوع تصویر...</option>`;const r=await fetch("/api/imagetypes"),x=await readApiJson(r);if(!r.ok||!x.success)throw new Error(apiErrorMessage(r,x,"انواع تصویر دریافت نشد."));(x.imageTypes||[]).forEach(t=>{const o=document.createElement("option");o.value=t.imageTypeID;o.textContent=t.imageTypeName;E.uploadImageType.appendChild(o);});}
