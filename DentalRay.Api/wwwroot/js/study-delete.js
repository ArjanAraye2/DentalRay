...
(function () {
    ...
    // Add selected FDI teeth to the existing New/Edit Study JSON requests without
    // duplicating the original form logic in app.js.
    const originalFetch=window.fetch.bind(window);
    window.fetch=async function(input,init){
        try{
            const url=typeof input==="string"?input:(input?.url||""); const method=(init?.method||"GET").toUpperCase();
            if(/^\/api\/radiologystudies(?:\/\d+)?$/.test(url)&&(method==="POST"||method==="PUT")&&typeof init?.body==="string"){
                const data=JSON.parse(init.body); const chart=document.getElementById(method==="POST"?"newStudyDentalChart":"editStudyDentalChart");
                data.toothNumbers=window.DentalRayDentalChart?.getSelected(chart)||[]; init={...init,body:JSON.stringify(data)};
            }
        }catch(e){console.error("DentalRay: could not add tooth selections to Study request.",e);}
        return originalFetch(input,init);
    };

    function enhanceStudyAccordions(){document.querySelectorAll(".study-card").forEach(card=>{if(card.dataset.accordionReady==="1")return;const header=card.querySelector(".study-card-header");if(!header)return;const title=header.querySelector(".study-title"),meta=card.querySelector(".study-meta");if(meta){const dateLine=Array.from(meta.children).find(x=>(x.querySelector("strong")?.textContent||"").includes("تاریخ"));const dateText=dateLine?.querySelector("span")?.textContent;if(title&&dateText)title.textContent=dateText;}const body=document.createElement("div");body.className="study-accordion-body hidden";Array.from(card.children).filter(x=>x!==header).forEach(x=>body.appendChild(x));const chart=document.createElement("div");chart.className="study-readonly-odontogram";body.insertBefore(chart,body.firstChild);card.appendChild(body);const arrow=document.createElement("span");arrow.className="study-accordion-arrow";arrow.textContent="⌄";header.insertBefore(arrow,header.firstChild);header.classList.add("study-accordion-header");header.addEventListener("click",async e=>{if(e.target.closest("button"))return;const opening=body.classList.contains("hidden");body.classList.toggle("hidden",!opening);card.classList.toggle("study-open",opening);arrow.textContent=opening?"⌃":"⌄";if(opening&&window.DentalRayDentalChart&&!chart.dataset.rendered){const teeth=await loadStudyTeeth(Number(card.dataset.studyId));window.DentalRayDentalChart.render(chart,teeth);chart.dataset.rendered="1";}});card.dataset.accordionReady="1";});}

    function wireFormCharts(){
        document.getElementById("newStudyButton")?.addEventListener("click",()=>setTimeout(()=>{const c=document.getElementById("newStudyDentalChart");if(c&&window.DentalRayDentalChart)window.DentalRayDentalChart.render(c,[]);},0));
        document.getElementById("studiesContainer")?.addEventListener("click",e=>{const button=e.target.closest("button");if(!button||button.textContent.trim()!=="ویرایش")return;const card=button.closest(".study-card"),id=Number(card?.dataset.studyId);if(!id)return;setTimeout(async()=>{const c=document.getElementById("editStudyDentalChart");if(c&&window.DentalRayDentalChart)window.DentalRayDentalChart.render(c,await loadStudyTeeth(id));},0);},true);
    }
